...
/*   Updated: 2024/06/29 22:59:18 by emgul            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_y(t_fdf *fdf, t_point *start_p, t_point *end_p, float angle)
{
	...
	start_p->z = (-r.sx * r.sin_ang + r.sz * r.cos_ang) / z_scale;
	end_p->x = r.ex * r.cos_ang + r.ez * r.sin_ang;
	end_p->z = (-r.ex * r.sin_ang + r.ez * r.cos_ang) / z_scale;
}
...
void	rotate_z(t_fdf *fdf, t_point *start_p, t_point *end_p, float angle)
{
	...
	z_scale = fdf->cam->z_scale_factor;
	calculate_trig(angle, &r.cos_ang, &r.sin_ang);
	r.sx = start_p->x * z_scale;
	r.sy = start_p->y * z_scale;
	r.ex = end_p->x * z_scale;
	r.ey = end_p->y * z_scale;
	start_p->x = (r.sx * r.cos_ang - r.sy * r.sin_ang) / z_scale;
	start_p->y = (r.sx * r.sin_ang + r.sy * r.cos_ang) / z_scale;
	end_p->x = (r.ex * r.cos_ang - r.ey * r.sin_ang) / z_scale;
	end_p->y = (r.ex * r.sin_ang + r.ey * r.cos_ang) / z_scale;
}
